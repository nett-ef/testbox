testbox
=======
`testbox` is an Ansible playbook that deploys a minimalistic single-node
Kubernetes cluster.
...
==================================
With the playbook, you will get a cloud-native environment that can come in
handy. The environment has zero reliability, and almost no security measures
are applied.
Study the topics to meet your SLOs and develop your security policy.

Requirements
============
The following system packages must be installed to follow the README:

*   `Ansible >= 2.13.2 <https://docs.ansible.com/ansible/latest/installation_guide/index.html>`_
*   `Helm >= 3.9.3 <https://helm.sh/docs/intro/install/>`_
*   `kubectl >= 1.24.2 <https://kubernetes.io/docs/tasks/tools/>`_

Usage
=====
#.  Configure the Ansible and create the inventory.
...
*   `common` - common administration tasks: create groups, add users to the
    groups, etc.
...
===========================
#.  Get the kubeconfig file from the server:

    ``% microk8s config view``
#.  Add `proxy-url:` key to the `-cluster` section of the `~/.kube/config` file,
    for example::
...
#.  Use SSH dynamic port forwarding, for example:
...
#.  Change the service type to `NodePort`, making it accessible outside the
    cluster.

    Documentation:
    `Type NodePort <https://kubernetes.io/docs/concepts/services-networking/service/#type-nodeport>`_
#.  Optionally change the service `externalTrafficPolicy` to `Local` to disable
    SNAT on the cluster network.

    Documentation:
    `Preserving the client source IP <https://kubernetes.io/docs/tasks/access-application-cluster/create-external-load-balancer/#preserving-the-client-source-ip>`_

Deployment example
==================
`Gitea <https://gitea.io/en-us/>`_ will be installed from the
`Helm chart <https://artifacthub.io/packages/helm/gitea/gitea>`_.
...
#.  Get `values.yaml` from the
    `helm-chart repo <https://gitea.com/gitea/helm-chart/src/branch/main/values.yaml>`_.
#.  Configure the application, for example: `environments/dev.example/k8s/gitea/values.yaml`.
...
----------------------
#.  Create `gitea-admin-secret` as stated in the `values.yaml`::

        kubectl create secret generic gitea-admin-secret \
          --from-file=username=./username.txt \
          --from-file=password=./password.txt

    Documentation: `Managing Secrets using kubectl <https://kubernetes.io/docs/tasks/configmap-secret/managing-secret-using-kubectl/>`_
#.  Create the application account and get the UID (`1001`).
#.  Set `podSecurityContext` and `containerSecurityContext`::
    ...
    Documentation: `Configure a Security Context for a Pod or Container <https://kubernetes.io/docs/tasks/configure-pod-container/security-context/>`_

Updating
--------
#.  Get updated `values.yaml` from the
    `helm-chart repo <https://gitea.com/gitea/helm-chart/src/branch/main/values.yaml>`_.
